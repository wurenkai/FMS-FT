...
parser.add_argument('--datapath', default=r"", help='data path')
...
for i in trange(len(test_limg)):
    ...
    pred_op2 += np.sum((pred_error > op_thresh2)) / np.sum(mask)
    pred_op1 += np.sum((pred_error > op_thresh1)) / np.sum(mask)
...
print("#### >2.0", pred_op2 / len(test_limg))
print("#### >1.0", pred_op1 / len(test_limg))